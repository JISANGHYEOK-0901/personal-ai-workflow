---
name: playboard
description: Builds and operates a PlayBoard — a registry-derived single source of truth (SoT) surface where planning, issues, implementation status, schedule, technical policy, and design reality converge into one living board. Reproduces the framework/theme-agnostic spec (6 registries → derived views, 10 routes, parallel-wave schedule, control-area policy hub, hybrid capture pipeline, integrity invariants, exposure gate) in any host project, and enforces the operating rules that keep the SoT alive (derive-only, same-PR simultaneous update, bidirectional sync, status-transition convention, board-health checks).
when_to_use: Use when the user wants to build, reproduce, or operate a "PlayBoard" / 기획·구현 상황판 / single-source-of-truth project board / living spec hub, or mentions "playboard", "/playboard", "상황판", "SoT 보드", "레지스트리 파생 보드", "구현 통계 매트릭스", "mission-critical 정책 허브", or wants planning + issues + implementation status + schedule + tech policy + design demos unified into one derived surface. Also use to audit an existing board's SoT integrity and board health.
---
