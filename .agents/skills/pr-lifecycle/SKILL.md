...
변경·검증 검토 → 최종 커밋과 설치 환경의 검토 증표 → push·PR 생성 → 승인된 머지 → base 동기화 → 해당 작업의 로컬·원격 브랜치 정리까지 이어지는 절차다. 사용자가 요청한 단계와 이미 승인한 범위에서 진행한다. 사용자가 이 개인 기본 설정을 채택한 환경에서 “PR해줘”는 검증·검토·PR 생성·머지·브랜치 정리까지의 요청으로 처리한다. “PR만 생성”, “머지하지 마”, “검토만”처럼 제한한 요청은 그 단계에서 종료한다. 다른 사용자의 환경이나 미채택 프로젝트에 이 권한 해석을 자동 이식하지 않는다. 머지 요청에는 아래 조건을 통과한 해당 작업 브랜치 정리가 포함되며 별도 반복 승인을 요구하지 않는다.
...
작업 브랜치에서 push·PR 생성 전에 base 대비 전체 diff와 실제 기능 영향을 [결정 분류와 diff 검토](../execute/references/decision-diff-review.md)에 따라 검토한다. staged·unstaged·관련 미추적 파일을 포함하고 각 변경을 요구·기결정에 연결한다. 테스트·CI·`git diff --check` 성공을 의미적 검토로 대신하지 않는다. 실제 검증 명령과 필요한 계약·통합·문서 검증 결과를 확인한다. 발견한 MINOR 결함은 허용된 범위에서 수정하고 변경 때문에 무효화된 검증과 최종 diff를 다시 확인한다. MAJOR 결정이나 BLOCKER는 의존 단계 전에 해결하며 필수 프로젝트 게이트는 생략하지 않는다.

검토한 관련 파일만 최종 커밋하고 커밋된 변경이 검토한 diff와 일치하는지 확인한다. PR 게이트 훅이 설치된 환경에서는 최종 커밋 뒤 현재 base/HEAD에 대한 검토 증표를 기록한 다음 해당 브랜치만 push한다. 증표는 실제 diff 검토를 대신하지 않으며 상태가 바뀌면 영향을 다시 검토하고 갱신한다. 기록 명령과 조건은 설치 환경 안내를 따른다. 이 템플릿 저장소에서는 `docs/COMMUNICATION_HOOKS.md`에 있으며, 훅이 없는 도입 환경에 훅 설치나 증표 생성을 강제하지 않는다.

검토와 push 뒤 PR을 생성한다. PR 본문은 문제·최종 동작·검증·미검증/한계를 설명하며 여러 줄 본문은 파일 또는 구조화 인자로 전달한다.
...
리뷰 근거를 저장소·base SHA·head SHA·검증 대상 통합 결과에 연결하고 생성 후 실제 PR 번호를 추가한다. PR의 CI 결과도 확인하며 CI가 없으면 미설정이라고 명시하고 통과로 꾸미지 않는다. 인간 승인이 필요한 정책이면 그것을 AI 자체 검토로 대체하지 않는다. PR 생성까지만 요청받았다면 URL과 검증 상태를 보고하고 종료한다.
...
3. 원격 head가 이미 없으면 삭제 완료로 처리한다. 존재하면 확인한 SHA를 조건으로 해당 ref만 삭제한다. PR 게이트 훅이 설치돼 있으면 설치 환경 안내에 따라 MERGED 상태와 정확한 head SHA에 연결된 정리 증표를 기록한다. 이는 PR 생성·머지용 diff 검토 증표와 별개다. Git에서는 `git push --force-with-lease=refs/heads/<head>:<확인SHA> <remote> :refs/heads/<head>`로 조회 후 전진한 브랜치를 삭제하지 않도록 한다. 오류 시 무조건 재시도하지 말고 현재 ref를 재확인한다.
