...
1. 호스트 프로젝트의 일반화 매핑(평면 2~4·산출물 화면·구현 상태 4단계·작업 DAG·제어 영역·흐름·Day1 anchor·노출 플래그)이 잡히는지 — 없으면 매핑부터 확정
2. 이미 PlayBoard가 있으면 재현(build)인지 운영(operate)·감사인지 — 감사면 무결성 불변식·Board Health 우선
3. 제1원칙(파생 only) 위반 여지(하드코딩 목록·두 곳 중복 기재)가 있는지

확인은 1턴에 묶어 처리한다. 6 레지스트리 SoT가 서기 전에는 파생 뷰를 단정하지 않는다.
