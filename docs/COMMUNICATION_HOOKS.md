...
직접 명령의 정적 인자는 단일·이중 인용, 인용 연결, backslash 인용, `$'…'`의 일반 문자와 `\\`, `\'`, `\"`, `\a`, `\b`, `\e`, `\E`, `\f`, `\n`, `\r`, `\t`, `\v` 이스케이프를 지원한다. 인용된 `';'`, `'>'`, 빈 문자열·개행은 인자이고 실제 연산자만 복합 명령·리다이렉션으로 분류한다. 리다이렉션은 연산자에 바로 붙은 인용되지 않은 파일 디스크립터 번호, 연산자, 대상을 실제 명령 인자와 분리하며 PR 명령과 함께 있으면 거부한다. 파일 디스크립터 번호와 연산자를 backslash-newline으로 이은 형태는 Bash·Zsh의 해석이 다를 수 있어, 어느 한쪽에서 직접 PR 명령이 실행될 수 있으면 보수적으로 거부한다. 대상 파일명인 `--help`·`--dry-run`은 면제 근거가 아니며 실제 명령 옵션만 도움말·dry-run 면제 대상으로 판단한다. ANSI-C 인용 heredoc delimiter도 같은 인용 규칙으로 읽는다.

숫자 ANSI-C 이스케이프는 값을 해석하지 않은 불확실한 토큰으로 보존하고 뒤의 정적 명령 구조도 끝까지 읽는다. 실행 파일이 불확실하거나 직접 git/gh의 인자 값이 불확실한 경우, 불확실한 토큰과 PR 경계가 함께 있는 호출은 PR 의도·검토 증표 유무와 무관하게 거부한다. `env` 옵션이 불확실한 경우에도 인자 소비 개수나 후속 실행 파일을 추정하지 않고 거부한다. `printf` 단독처럼 실행 파일·구조가 확정되고 불확실성이 비PR 인자 값에만 있으면 기존 상기를 유지한다. 인용·heredoc·리다이렉션 경계를 확정할 수 없는 어휘·구조 해석 실패와 복구 불가능한 미지원 ANSI-C 이스케이프는 원문 정규식으로 PR 후보를 추정하지 않고 거부하며 literal 인자·`--body-file` 등으로 단순화하도록 안내한다. 이 거부는 Bash/Zsh에서 실행 불가능하다는 판정이 아니다. 정상 인용 예제·heredoc 본문·실제 도움말은 기존처럼 처리한다. 이 파서는 Bash/Zsh 전체 문법을 구현하지 않으며 명령·변수 치환, 동적으로 만든 실행 파일 이름, 래퍼의 내부 실행은 아래 미지원 범위에 해당한다.
