...
- 직접 명령의 backslash-newline과 도움말 옵션을 구분한다. 출력 전용 실행 파일로 실제 셸 argv와 heredoc 뒤 명령 도달을 대조하고, 인용·주석·구분자 앞/중간/종료 줄 연속·홀짝 백슬래시·탭 제거 경계를 검사한다. 종료 구분자의 줄 연결은 macOS `/bin/sh`와 Ubuntu의 dash가 다르므로 이 사례는 `/bin/bash`를 명시하고, 나머지 이식 가능한 사례는 `/bin/sh`로 검사한다. 제목·본문 값인 `--help`, false 값, 도움말과 실제 쓰기가 섞인 명령은 검토를 면제하지 않는다.
...
## PR 명령 구조와 불확실성 처리 보완 (2026-09-18)

- 기준은 develop `ce02fa6da6d644d5f4afca920aa7779309ec3cc1` 위 `fix/pr-shell-command-structure` 변경이다. PR #15 이후 재현한 기존 리다이렉션 누락과 파싱 중단 뒤 정적 인용 PR 명령 누락을 보완했다.
- 실행 파일·실제 인자와 리다이렉션의 FD·연산자·대상을 분리하여 도움말/dry-run 면제에는 실제 인자만 사용한다. 숫자 ANSI-C 값은 불확실성을 유지한 채 뒤의 명령 구조를 계속 읽으며, 구조 자체를 확인할 수 없으면 원문 정규식으로 안전을 추정하지 않고 거부한다. 독립 검토에서 발견한 불확실한 env 옵션의 인자 소비 문제도 차단한다.
- 표 기반 회귀 네 개로 위치·인용·파일명·옵션·불확실성 전후의 조합과 정상 읽기·인용 예제·heredoc 대조군을 묶었다. 출력 전용 stub으로 실제 Bash/Zsh argv를 비교한다. FD 줄 연속은 두 셸의 서로 다른 argv를 명시하고, Bash에서 직접 PR 실행이 가능한 경우 보수적으로 거부한다. Zsh가 없는 환경은 해당 대조만 생략하며 로컬에서는 두 셸 모두 실행했다.
- 전체 unittest **87개 PASS**(기존 83개 + 표 기반 회귀 4개), PR gate 집중 34개와 communication 31개 PASS. 스킬 동기화·저장소 정합성·공백 검사 PASS. 실제 임시 설치 runtime CLI의 두 엔진과 증표 없음/유효/무효 상태에서도 기존 두 누락 및 env 반례의 거부와 정상 push의 유효 증표 경로를 확인했다.
- **한계:** 이 결과는 명시한 직접 명령·실패 계약의 검증이다. Bash/Zsh 전체 문법, 동적 치환·임의 래퍼 내부, 실제 모델의 판단 품질, 원격 조회 이후 경쟁의 원자성을 보장하지 않는다. 훅 설치·재신뢰·PR 실행 결과는 작업 기록과 해당 PR에 연결한다.
