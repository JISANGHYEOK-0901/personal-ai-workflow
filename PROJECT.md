...
- 선택형 훅: `hooks/communication.py` 정본을 `scripts/install_communication_hooks.py`로 Codex·Claude에 로컬 설치한다. 일반 전달 점검은 advisory이고, PR 요청 턴의 push와 직접 PR 생성·머지는 현재 base/HEAD 검토 증표를 요구한다. merge는 명시 PR 번호·원격 base/head·CI를 추가 확인한다. 절차·한계·활성화는 `docs/COMMUNICATION_HOOKS.md`. 추가 검증은 `python3 -m unittest discover -s tests -p 'test_communication_hooks.py' -v`; 전체 unittest에도 포함된다.
