...
- 선택형 훅: `hooks/communication.py` 정본을 `scripts/install_communication_hooks.py`로 Codex·Claude에 로컬 설치한다. 절차·한계·활성화는 `docs/COMMUNICATION_HOOKS.md`. 추가 검증은 `python3 -m unittest discover -s tests -p 'test_communication_hooks.py' -v`; 전체 unittest에도 포함된다.
